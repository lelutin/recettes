...
* Il est suggéré d'utiliser des nouilles ramen, mais n'importe quel type
  de nouilles sèches peut fonctionner.
* La sorte de viande peut aussi être modifiée au goût.
* Pour accélérer la préparation et donner un meilleur goût au poulet, on peut
  précuire le poulet en le faisant saisir dans une poêle quelques jours avant.
  On voudra alors surtout bien réchauffer le poulet dans le bouillon au lieu de
  le faire cuire.
* Les crevettes sont vraiment meilleures si mangées tout de suite après la
  cuisson -- ça n'est pas très bon réchauffé. Donc prévoir la qté pour la
  cuisson selon le nb de portions qui seront servies.
* Comme pour les crevettes, c'est meilleur si on fait cuire les nouilles juste
  avant de servir, donc prévoir seulement la qté de nouilles pour le nombre de
  portions qu'on veut servir.
